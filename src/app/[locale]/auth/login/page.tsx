...
export const metadata: Metadata = {
  title:  'Connexion — Mathieu&Co Studio',
  robots: 'noindex',
};
    ...
    <div className="min-h-[100dvh] bg-beige flex flex-col items-center justify-center px-4 py-16">
      <div className="w-full max-w-[420px]">
        ...
        <div className="rounded-sm border border-gris-cl bg-blanc p-8 shadow-sm">
          <h1 className="text-[18px] font-semibold text-noir mb-1.5">Se connecter</h1>
          <p className="text-[11.5px] text-gris mb-7">
            Accédez à votre espace personnel, suivez vos commandes et gérez vos favoris.
          </p>

          {/* Erreur OAuth */}
          {error && (
            <div className="mb-5 rounded-sm bg-rouge/8 border border-rouge/20 px-4 py-3 text-[11px] text-rouge">
              {decodeURIComponent(error)}
            </div>
          )}

          <AuthForm mode="login" redirectTo={redirectTo ?? '/compte'} />

          {/* Lien register */}
          <p className="mt-6 text-center text-[11px] text-gris">
            Pas encore de compte ?{' '}
            <Link href="/auth/register" className="font-semibold text-noir hover:text-or transition-colors">
              Créer un compte
            </Link>
          </p>
        </div>
